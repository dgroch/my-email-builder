...
//
// Cervanttis is a script face used only for accents, captions and signatures, and the brand sets
// it lowercase everywhere. NeuzeitGro carries text-transform:uppercase at every size it is used
// at, so the authored casing cannot reach the render either way.
//
// Lust is deliberately NOT in here, and the reason is the general one: a face tells you how
// something is set, not what it is. Lust is the display serif for headlines and pull-quotes —
// and also for the price on a product card, the code in a promo box, and the numeral on a step.
// "Sentence case" is a claim about prose, so inferring it from the typeface rejects `20% off`,
// `from $85` and `bloom20`, and then offers `20% Off`, `From $85` and `Bloom20` as the repair.
// On a coupon code that suggestion is not just noise: applying it breaks the code.
//
// Nothing is lost by leaving it out. Every Lust token that genuinely wants Sentence case already
// says so in its own TOKENS line — HEADLINE, PRODUCT_NAME, PULL_QUOTE, SECTION_HEADLINE and the
// tile titles all carry "SENTENCE CASE" and are picked up above, before this map is consulted.
// The fallback only ever reached the tokens whose descriptions omit a casing rule *because they
// do not have one*.
const FONT_CASE = { cervanttis: 'lower', neuzeitgro: 'any' };
...
function caseRule(name, desc, tokenRules, font) {
  ...
  // `|| null` keeps the field's shape stable: a face with no imposed rule (Lust) has to serialise
  // as an explicit null, the same as a token that renders in no brand face at all, rather than
  // vanishing from the JSON as undefined.
