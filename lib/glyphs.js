...
// ── is the guard actually on? ─────────────────────────────────────────────────────────
//
// Every read path above returns null or empty rather than throwing, which is right: a malformed
// subtable must not take down a render. But it means the fold check degrades to "nothing is
// wrong with any of it", which is the exact shape of the fault this module exists to catch —
// Cervanttis failed open for months precisely because nothing errored.
//
// So the silence has to be reportable. This says whether the guard is actually holding, and the
// callers that can act on it (startup, /api/health, /api/validate) say so out loud.
const EXPECTED_FACES = ['cervanttis', 'lust', 'neuzeitgro'];

function gateStatus() {
  let loaded = [];
  try { loaded = Object.keys(faces()); } catch (_) { loaded = []; }
  const missing = EXPECTED_FACES.filter((k) => !loaded.includes(k));
  return {
    ok: missing.length === 0,
    loaded, missing,
    reason: missing.length === 0 ? null
      : `${missing.join(', ')} could not be read from ${path.relative(path.join(__dirname, '..'), SHELL)}, so unsupported_glyph cannot fire for ${missing.length > 1 ? 'those faces' : 'that face'}.`,
  };
}

module.exports = { auditCampaign, inspect, faceCoverage, facesThatCanSet, faces, gateStatus, readCmap, baseLetter, EXPECTED_FACES };
