...
async function gather({ forceFresh = false, brief = '' } = {}) {
  ...
  _inflight = (async () => {
    ...
    const [shopRes, klavRes, notionRes, imgRes] = await Promise.all([
      ...
      briefAgnostic ? { ok: true, blogPosts: briefAgnostic.blogPosts } : gatherNotionBlog(),
    ...
    const blogPosts = (briefAgnostic && briefAgnostic.blogPosts) || (notionRes.ok ? notionRes.blogPosts : []);
