...
const SHOPIFY_BASE = process.env.SHOPIFY_STORE_DOMAIN || 'figandbloom.com';
...
// Shopify blog index — the store's blog handle is `journal`, so the Atom feed is at
// /blogs/journal.atom. Public, no auth. Top 20 most recent.
//
// This pointed at /blogs/news.atom, which 404s (and did on both domains — it was never a
// domain problem). The fetch degrades silently on failure, so blog context has been coming
// back empty rather than erroring. Old *article* URLs under /blogs/news/… still resolve,
// which is why the exemplar tile links work; only the feed index moved.
const SHOPIFY_BLOG_FEED_URL = (process.env.SHOPIFY_BLOG_FEED_URL
  || 'https://figandbloom.com/blogs/journal.atom');
    ...
    const products = (data.products || [])
      ...
      .map((p) => {
        ...
        return {
          ...
          url: 'https://figandbloom.com/products/' + p.handle,
        };
      });
