'use strict';
// klaviyo.js — push an assembled campaign to Klaviyo as a *draft* email campaign.
// Zero-dependency: talks to the Klaviyo REST API over Node's built-in https.
//
// Flow (all draft, nothing is ever sent):
//   1. POST /api/templates/                       → create a CODE template from the HTML
//   2. POST /api/campaigns/                        → create the draft campaign (+ its message)
//   3. POST /api/campaign-message-assign-template/ → attach the template to the message
//
// Auth: a Klaviyo *private* API key with `campaigns:write` + `templates:write` scopes,
// supplied to the server as the KLAVIYO_API_KEY env var (never sent to the browser).

const https = require('https');

const HOST = 'a.klaviyo.com';
// Pin an API revision so payload shapes stay stable. Override with KLAVIYO_REVISION.
const REVISION = process.env.KLAVIYO_REVISION || '2026-04-15';

function request(method, apiPath, apiKey, body) {
  return new Promise((resolve, reject) => {
    const payload = body ? JSON.stringify(body) : null;
    const headers = {
      'Authorization': 'Klaviyo-API-Key ' + apiKey,
      'Accept': 'application/vnd.api+json',
      'revision': REVISION,
    };
    if (payload) {
      headers['Content-Type'] = 'application/vnd.api+json';
      headers['Content-Length'] = Buffer.byteLength(payload);
    }
    const req = https.request({ hostname: HOST, path: apiPath, method, headers }, (res) => {
      let data = '';
      res.on('data', (c) => { data += c; });
      res.on('end', () => {
        let parsed = null;
        try { parsed = data ? JSON.parse(data) : null; } catch (_) { /* non-JSON error body */ }
        if (res.statusCode >= 200 && res.statusCode < 300) return resolve(parsed);
        const detail = parsed && parsed.errors && parsed.errors[0] && (parsed.errors[0].detail || parsed.errors[0].title);
        reject(new Error(`Klaviyo ${method} ${apiPath} → ${res.statusCode}${detail ? ': ' + detail : ': ' + data.slice(0, 400)}`));
      });
    });
    req.on('error', reject);
    if (payload) req.write(payload);
    req.end();
  });
}

// Upload one optimised image to the Klaviyo media library (multipart/form-data) and return its
// hosted CDN URL (data.attributes.image_url). Requires the `images:write` scope.
//
// The buffer arrives from the optimisation stage, so it is no longer always a PNG: opts carries
// the chosen encoding's extension and content type, and both the filename and the multipart
// part header must match the bytes or Klaviyo (and the receiving mail clients) will mis-handle
// the file. Defaults to PNG so an un-optimised caller still behaves exactly as before.
function uploadImage(apiKey, buffer, name, opts = {}) {
  return new Promise((resolve, reject) => {
    const boundary = '----ebslice' + Math.random().toString(36).slice(2);
    const safe = (name || 'slice').replace(/[^\w.\-]+/g, '-');
    const ext = String(opts.ext || 'png').toLowerCase().replace(/[^a-z0-9]/g, '') || 'png';
    const mime = opts.mime
      || (ext === 'jpg' || ext === 'jpeg' ? 'image/jpeg' : ext === 'webp' ? 'image/webp' : 'image/png');
    const head = Buffer.from(
      `--${boundary}\r\n` +
      `Content-Disposition: form-data; name="file"; filename="${safe}.${ext}"\r\n` +
      `Content-Type: ${mime}\r\n\r\n`, 'utf8');
    const tail = Buffer.from(
      `\r\n--${boundary}\r\n` +
      `Content-Disposition: form-data; name="name"\r\n\r\n${safe}\r\n` +
      `--${boundary}--\r\n`, 'utf8');
    const body = Buffer.concat([head, buffer, tail]);
    const headers = {
      'Authorization': 'Klaviyo-API-Key ' + apiKey,
      'Accept': 'application/vnd.api+json',
      'revision': REVISION,
      'Content-Type': 'multipart/form-data; boundary=' + boundary,
      'Content-Length': body.length,
    };
    const req = https.request({ hostname: HOST, path: '/api/image-upload', method: 'POST', headers }, (res) => {
      let data = '';
      res.on('data', (c) => { data += c; });
      res.on('end', () => {
        let parsed = null;
        try { parsed = data ? JSON.parse(data) : null; } catch (_) {}
        const url = parsed && parsed.data && parsed.data.attributes && parsed.data.attributes.image_url;
        if (res.statusCode >= 200 && res.statusCode < 300 && url) return resolve(url);
        const detail = parsed && parsed.errors && parsed.errors[0] && (parsed.errors[0].detail || parsed.errors[0].title);
        reject(new Error(`Klaviyo image-upload → ${res.statusCode}${detail ? ': ' + detail : ': ' + data.slice(0, 300)}`));
      });
    });
    req.on('error', reject);
    req.write(body);
    req.end();
  });
}

function escAttr(s) { return String(s == null ? '' : s).replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;'); }

// Build one email table row for a hosted image slice — optionally wrapped in a link so
// the block is individually clickable (the "each block has its own URL" requirement).
function imageRow(imageUrl, opts = {}) {
  const alt = escAttr(opts.alt || '');
  const img = `<img src="${escAttr(imageUrl)}" alt="${alt}" width="600" style="display:block;width:100%;max-width:600px;height:auto;border:0;outline:none;text-decoration:none;">`;
  const cell = opts.href
    ? `<a href="${escAttr(opts.href)}" target="_blank" style="text-decoration:none;display:block;">${img}</a>`
    : img;
  return `<tr><td style="padding:0;font-size:0;line-height:0;">${cell}</td></tr>`;
}

// Build one email row of SIDE-BY-SIDE image columns — how a layout:'cols' block (a live
// animated GIF column with rasterised PNG content beside it, e.g. blocks/image-text) is
// recomposed on push. cells = [{url, widthPx, pct, bg}] in left-to-right order. Percentage
// cell widths + width:100% images keep the columns proportional when the 600px shell scales
// to 100% on mobile. valign:middle + the block's background colour absorb the height
// difference when the GIF's natural ratio differs from the rasterised column's height.
function columnRow(cells, opts = {}) {
  const alt = escAttr(opts.alt || '');
  const tds = (cells || []).map((c) => {
    const img = `<img src="${escAttr(c.url)}" alt="${alt}" width="${Math.round(c.widthPx || 0)}" style="display:block;width:100%;height:auto;border:0;outline:none;text-decoration:none;">`;
    const inner = opts.href
      ? `<a href="${escAttr(opts.href)}" target="_blank" style="text-decoration:none;display:block;">${img}</a>`
      : img;
    const bg = c.bg ? `bgcolor="${escAttr(c.bg)}" ` : '';
    return `<td width="${c.pct}%" ${bg}valign="middle" style="width:${c.pct}%;padding:0;font-size:0;line-height:0;vertical-align:middle;">${inner}</td>`;
  }).join('');
  return `<tr><td style="padding:0;font-size:0;line-height:0;"><table width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;border-collapse:collapse;"><tr>${tds}</tr></table></td></tr>`;
}

// Fetch every page of a JSON:API collection (follows links.next), returning all data items.
async function getAll(apiPath, apiKey) {
  const items = [];
  let next = apiPath;
  let guard = 0;
  while (next && guard++ < 50) {
    const page = await request('GET', next, apiKey);
    if (page && Array.isArray(page.data)) items.push(...page.data);
    const link = page && page.links && page.links.next;
    if (!link) break;
    // links.next is an absolute URL; reduce it to the path+query our request() expects.
    try { const u = new URL(link); next = u.pathname + u.search; } catch (_) { next = link; }
  }
  return items;
}

// List the account's audiences (lists + segments) as { lists:[{id,name}], segments:[{id,name}] }.
// Used to power an audience picker so users select by name instead of guessing the ID.
async function listAudiences(apiKey) {
  if (!apiKey) throw new Error('KLAVIYO_API_KEY is not configured on the server.');
  const map = (arr) => arr
    .map((d) => ({ id: d.id, name: (d.attributes && d.attributes.name) || d.id }))
    .sort((a, b) => a.name.localeCompare(b.name));
  const [lists, segments] = await Promise.all([
    getAll('/api/lists/', apiKey),
    getAll('/api/segments/', apiKey),
  ]);
  return { lists: map(lists), segments: map(segments) };
}

// Guard: refuse to create a draft whose inbox line would be scraped rather than chosen.
// The subject has no fallback (the internal campaign name — "F&B | 2026-08 …" — is a label,
// not a subject), and preview text is mandatory because Klaviyo does NOT inject the
// preview_text field into CODE-editor templates: it must be baked into the HTML preheader,
// and an empty value leaves Gmail/Apple Mail to scrape the snippet from image alt text.
function assertSendReady({ subject, previewText } = {}) {
  if (!subject || !String(subject).trim()) {
    throw new Error('A subject line is required — the internal campaign name is not used as a fallback subject.');
  }
  if (!previewText || !String(previewText).trim()) {
    throw new Error('Preview text is required — Klaviyo does not inject preview_text into CODE-editor templates, so an empty value leaves the inbox snippet to be scraped from the email body (image alt text).');
  }
}

// Create a draft email campaign in Klaviyo from already-assembled HTML.
// opts: { apiKey, listId, name, subject, previewText, fromEmail, fromLabel, replyToEmail, html }
// `subject` and `previewText` are REQUIRED (see assertSendReady) — the caller is expected to
// have baked the same previewText into the HTML's hidden preheader (render.wrapProductionShell).
async function createDraftCampaign(opts = {}) {
  const { apiKey, listId, name, subject, previewText, fromEmail, fromLabel, replyToEmail, html } = opts;
  if (!apiKey) throw new Error('KLAVIYO_API_KEY is not configured on the server.');
  if (!listId) throw new Error('A Klaviyo audience (list or segment ID) is required.');
  if (!fromEmail) throw new Error('A "from" email address is required.');
  if (!html) throw new Error('Nothing to push — assembled HTML is empty.');
  assertSendReady({ subject, previewText });

  const campaignName = name || 'Untitled campaign';

  // 1. Template (raw HTML → CODE editor).
  const tpl = await request('POST', '/api/templates/', apiKey, {
    data: {
      type: 'template',
      attributes: {
        name: `${campaignName} (email builder ${new Date().toISOString().slice(0, 16).replace('T', ' ')})`,
        editor_type: 'CODE',
        html,
      },
    },
  });
  const templateId = tpl && tpl.data && tpl.data.id;
  if (!templateId) throw new Error('Klaviyo did not return a template id.');

  // 2. Draft campaign with a single email message. No send_strategy ⇒ stays a draft.
  // preview_text is set for Klaviyo's UI, but the copy that actually reaches inboxes is the
  // preheader baked into the HTML — if you change one, re-push so the other stays in sync.
  const content = {
    subject: String(subject).trim(),
    preview_text: String(previewText).trim(),
    from_email: fromEmail,
    from_label: fromLabel || fromEmail,
  };
  if (replyToEmail) content.reply_to_email = replyToEmail;

  const camp = await request('POST', '/api/campaigns/', apiKey, {
    data: {
      type: 'campaign',
      attributes: {
        name: campaignName,
        audiences: { included: [listId], excluded: [] },
        'campaign-messages': {
          data: [{
            type: 'campaign-message',
            attributes: { definition: { channel: 'email', label: campaignName, content } },
          }],
        },
      },
    },
  });
  const campaignId = camp && camp.data && camp.data.id;
  if (!campaignId) throw new Error('Klaviyo did not return a campaign id.');

  // Resolve the auto-created message id (from the create response, else fetch it).
  let messageId =
    camp.data.relationships &&
    camp.data.relationships['campaign-messages'] &&
    camp.data.relationships['campaign-messages'].data &&
    camp.data.relationships['campaign-messages'].data[0] &&
    camp.data.relationships['campaign-messages'].data[0].id;
  if (!messageId) {
    const msgs = await request('GET', `/api/campaigns/${campaignId}/campaign-messages/`, apiKey);
    messageId = msgs && msgs.data && msgs.data[0] && msgs.data[0].id;
  }
  if (!messageId) throw new Error('Could not resolve the campaign message id from Klaviyo.');

  // 3. Attach the template to the message so the draft opens with our HTML.
  await request('POST', '/api/campaign-message-assign-template/', apiKey, {
    data: {
      type: 'campaign-message',
      id: messageId,
      relationships: { template: { data: { type: 'template', id: templateId } } },
    },
  });

  return {
    campaignId,
    messageId,
    templateId,
    editUrl: `https://www.klaviyo.com/campaign/${campaignId}/edit`,
  };
}

module.exports = { createDraftCampaign, assertSendReady, uploadImage, imageRow, columnRow, listAudiences, REVISION };
