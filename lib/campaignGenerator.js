...
function renderLiveContextBlock(ctx) {
  ...
  if (Array.isArray(ctx.images) && ctx.images.length) {
    lines.push('');
    lines.push('LIFESTYLE IMAGES (semantic search results, semantically ranked for the brief; each URL is a Cloudflare R2 URL directly usable in image tokens like HERO_IMAGE_URL, POLAROID_IMAGE_URL, PHOTO_1_URL, FEATURE_1_IMAGE_URL, etc.):');
    for (const img of ctx.images.slice(0, 24)) {
      const desc = (img.description || '').replace(/\s+/g, ' ').slice(0, 200);
      lines.push('- ' + (img.title || '(untitled)') + (desc ? ' — ' + desc : '') + '\n    url: ' + img.url);
    }
    if (ctx.images.length > 24) lines.push('  … and ' + (ctx.images.length - 24) + ' more');
  } else if (status.images === 'unavailable') {
    lines.push('');
    lines.push('LIFESTYLE IMAGES: // unavailable — asset library unreachable, or the brief was empty when the search was run.');
  }
  return lines.join('\n');
}
