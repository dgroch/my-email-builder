...
/** Substitute the brief and audience into the user template. */
function renderUserPrompt(brief, audience) {
  return USER_TEMPLATE
    .replace('{{BRIEF}}', (brief || '').trim())
    .replace('{{AUDIENCE}}', (audience || 'default — RH | All Email Subscribers').trim());
}
...
async function generate({ brief, audience, maxTokens, temperature }) {
  if (!brief || !brief.trim()) {
    const err = new Error('Brief is empty. Tell me what the campaign is for.');
    err.code = 'EMPTY_BRIEF';
    throw err;
  }
  const user = renderUserPrompt(brief, audience);
...
async function generateValidated({ brief, audience, validateFn, maxTokens, temperature }) {
  let parsed = await generate({ brief, audience, maxTokens, temperature });
  ...
  if (validateFn) {
    ...
    const user2 = renderUserPrompt(brief, audience)
