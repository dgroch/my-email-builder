...
//
// `options.notUploadedIndexes`, when given, is a Set of block indexes whose slices are
// rasterised and optimised (so a preview can still show them) but are never uploaded to
// Klaviyo — the caller keeps them as live HTML instead (the footer, or an html_only
// component). Those records are tagged `uploaded: false` and excluded from the weight totals
// and the budget verdict, so an email can never be warned or refused for bytes Klaviyo will
// never receive.
async function optimiseSlices(slices, options = {}) {
  const cfg = options.config || config(options);
  const log = loggerFor(cfg);
  const notUploaded = options.notUploadedIndexes || null;
  const out = [];
  const records = [];
  for (const s of slices || []) {
    const label = sliceBaseName(s);
    const uploaded = !(notUploaded && notUploaded.has(s.index));
    if (s.kind === 'gif' || !s.buffer) {
      const rec = { label, component: s.component, index: s.index, seg: s.seg, name: s.name,
        kind: s.kind || 'gif', format: 'gif', before: 0, after: 0, passthrough: true, uploaded, cacheHit: false };
      records.push(rec);
      out.push({ ...s, image: rec });
      continue;
    }
    // One unreadable/unencodable slice must not take down the whole email: fall back to the
    // original rasterised bytes (already a valid PNG — it's a Puppeteer screenshot) with a
    // warning, rather than letting the error propagate into an unhandled 500 that would also
    // leak the failure's stack trace to the client.
    let res;
    try {
      res = await optimiseImage(s.buffer, { ...options, config: cfg, label });
    } catch (e) {
      log.warn(`[image-opt] optimisation failed for ${label}, shipping the unoptimised original: ${(e && e.message) || e}`);
      const rec = {
        label, component: s.component, index: s.index, seg: s.seg, segCount: s.segCount, name: s.name,
        kind: 'png', format: 'png', ext: 'png', mime: 'image/png',
        before: s.buffer.length, after: s.buffer.length, savedBytes: 0, savedPct: 0,
        ssim: 1, lossless: true, cacheHit: false, uploaded,
  ...
  // Every total and the budget verdict below are computed over what actually ships.
  const weighed = records.filter((r) => r.uploaded);
  const totalBefore = weighed.reduce((n, r) => n + r.before, 0);
  const totalAfter = weighed.reduce((n, r) => n + r.after, 0);
  const budget = assessWeight(weighed, cfg);
