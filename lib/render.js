...
const SHELL_TOKENS = new Set(['CAMPAIGN_NAME', 'BODY_BG', 'FONT_CDN_LINK', 'COMPONENTS', 'PREHEADER']);

function read(p) { return fs.readFileSync(p, 'utf8'); }

// Component HTML may come from a template file or from a version published in the Studio.
// templateSource is the single place that decides which; everything below just asks it.
const templateSource = require('./templateSource');
// Brand palette overrides are applied to the finished document rather than baked into
// templates, so one edit reaches shipped and authored components alike without rewriting an
// immutable published version. See lib/brandTokens.js.
const brandTokens = require('./brandTokens');
...
// Which tokens of a component are CSS lengths, read off the same schema /api/schema serves.
// Lazily required so the module graph stays acyclic and the schema is built at most once.
let _lengthIndex = null;
const NO_LENGTHS = new Set();
function lengthTokensOf(component) {
  if (!_lengthIndex) {
    _lengthIndex = new Map();
    for (const c of require('./parseTemplates').buildSchema(DS).components) {
      const lengths = c.tokens.filter((t) => t.type === 'length').map((t) => t.name);
      if (lengths.length) _lengthIndex.set(c.name, new Set(lengths));
    }
  }
  return _lengthIndex.get(component) || NO_LENGTHS;
}

// Outlook cannot shrink-wrap a VML roundrect to its label, so BTN_WIDTH exists purely to size
// that fallback. `auto` means "as wide as the label needs", which we resolve here to a real px
// value: NeuzeitGro Bold at 10px with .16em tracking runs ~7.8px per uppercase character, plus
// the button's 40px horizontal padding on each side. Every non-Outlook client ignores BTN_WIDTH
// and shrink-wraps the live <a> anyway, so this estimate only ever decides the Outlook box.
function autoButtonWidth(label) {
  const chars = String(label == null ? '' : label).trim().length;
  return Math.min(520, Math.max(120, Math.round(chars * 7.8) + 80)) + 'px';
}

// A bare number in a length token is a unit slip with an unambiguous intent. Left alone it
// interpolates to `padding:100 72px 100` — an invalid shorthand the browser drops whole, so the
// padding silently renders as 0. Coerce it to px here and let the validator report the warning.
function coerceLengths(component, tokens) {
  const lengths = lengthTokensOf(component);
  if (!lengths.size) return tokens;
  for (const name of lengths) {
    const v = tokens[name];
    if (typeof v !== 'string') continue;
    const t = v.trim();
    // A bare `0` is already a valid CSS length and templates use it to mean "no edge here" —
    // rewriting it to `0px` would churn every padding shorthand for nothing.
    if (t !== '0' && /^\d+(?:\.\d+)?$/.test(t)) tokens[name] = `${t}px`;
    else if (name === 'BTN_WIDTH' && t.toLowerCase() === 'auto') tokens[name] = autoButtonWidth(tokens.CTA_TEXT);
  }
  return tokens;
}

// The background a block paints behind itself. Every template opens with its outer
// `<tr><td style="…background:…">` (or an immediate bgcolor attribute), so the first colour
// declaration in the assembled block is the block's own panel colour.
function blockBackground(html) {
  const m = String(html || '').match(/background(?:-color)?\s*:\s*(#[0-9a-fA-F]{3,8})|bgcolor="(#[0-9a-fA-F]{3,8})"/);
  return m ? (m[1] || m[2]) : null;
}

// Components whose panel colour is almost always "whatever the block above me is using".
// sections/button draws its own full-width band, so a PANEL_BG that does not match its
// neighbour reads as a stray stripe rather than as a button on a panel — and the author has to
// know, and repeat by hand, a colour they already chose one block earlier.
const INHERITS_PANEL_BG = new Set(['sections/button']);
...
//
// ctx (optional) carries the assembly position: { prevBg, bodyBg }. It is what lets a blank
// PANEL_BG resolve to the preceding block's panel instead of to nothing.
function withDefaults(component, tokens, ctx) {
  const defaults = tokenDefaults()[component];
  const out = defaults ? { ...defaults } : {};
  for (const [k, v] of Object.entries(tokens || {})) {
    if (defaults && Object.prototype.hasOwnProperty.call(defaults, k) && (v == null || String(v).trim() === '')) continue;
    out[k] = v;
  }
  if (INHERITS_PANEL_BG.has(component) && (out.PANEL_BG == null || String(out.PANEL_BG).trim() === '')) {
    out.PANEL_BG = (ctx && (ctx.prevBg || ctx.bodyBg)) || '#ffffff';
  }
  return coerceLengths(component, out);
}
...
// Hidden inbox-preview ("preheader") block, injected as the first thing inside <body>.
// Klaviyo does NOT inject a campaign's preview_text into CODE-editor (raw HTML) templates —
// the preview_text field only feeds Klaviyo's own UI. Gmail / Apple Mail build their inbox
// snippet from the first text they can scrape out of the body, and a sliced email's first
// scrapable text is the first image's alt attribute. Baking the preview into the HTML is the
// only reliable way to control the snippet. The &zwnj;&nbsp; run pads out the remainder of
// the snippet so the scraper doesn't append body text (or alt text) after the preview line.
function preheader(text) {
  const t = text == null ? '' : String(text).trim();
  if (!t) return '';
  // Escape the text, but leave Klaviyo template tags ({% … %}, {{ … }}) verbatim: a dynamic
  // preview such as `{% if s|length > 35 %}` stops parsing once its `>` becomes `&gt;`.
  const escText = (x) => x.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const esc = t.split(/(\{%[\s\S]*?%\}|\{\{[\s\S]*?\}\})/).map((part, i) => (i % 2 ? part : escText(part))).join('');
...
// opts.previewText = inbox preview line, baked into the shell as a hidden preheader
// opts.shell = 'production' to wrap in the SENDABLE shell; anything else uses the preview shell.
//
// This is deliberately NOT the same switch as `production`, which only means "keep the real
// merge tags". The two come apart on the Klaviyo push, which needs both: real tags AND the
// preview shell, because that HTML is rasterised and the preview shell carries the fonts as
// base64 — the exact bytes renderToPng draws with. Point that path at the production shell and
// the slices would render in whatever the CDN happened to serve, or in a fallback face.
//
// The distinction is worth the extra option because getting it wrong is invisible. The preview
// shell is ~263KB of embedded font before a single word of copy, and it carries no
// `color-scheme: light` — so HTML built with it is clipped by Gmail inside the <head>, and
// inverts in dark mode exactly as shell-production.html was written to prevent.
function assemble(campaign, opts = {}) {
  const assetsBase = opts.assetsBase || '/design-system/assets';
  const shellFile = opts.shell === 'production' ? 'shell-production.html' : 'shell-preview.html';
  const shell = read(path.join(DS, 'shell', shellFile));
  const parts = [];
  const unfilled = [];

  // The panel colour of the block assembled immediately before this one — see INHERITS_PANEL_BG.
  let prevBg = null;
  const bodyBg = campaign.bodyBg || '#2c2825';

  (campaign.blocks || []).forEach((block, i) => {
    // opts.templateOverrides lets the Studio preview a component that is not saved or published
    // yet — the working canvas is compiled in memory and handed straight to the real assembler,
    // so what the designer previews is the same pipeline a campaign uses.
    const override = opts.templateOverrides && opts.templateOverrides[block.component];
    const src = override ? { html: override } : templateSource.resolve(block.component);
    if (!src) { unfilled.push({ component: block.component, token: '(missing template)' }); return; }
    let html = src.html;
    html = applyTokens(html, withDefaults(block.component, block.tokens, { prevBg, bodyBg }), assetsBase);
    prevBg = blockBackground(html) || prevBg;
  ...
  full = full
    ...
    .split('{{PREHEADER}}').join(preheader(opts.previewText));

  return { html: brandTokens.applyOverrides(full), unfilled };
}
...
function assembleBlocks(campaign, opts = {}) {
  ...
  (campaign.blocks || []).forEach((block, index) => {
    const src = templateSource.resolve(block.component);
    if (!src) return;
    const tokens = withDefaults(block.component, block.tokens, { prevBg, bodyBg });
    const html = brandTokens.applyOverrides(applyTokens(src.html, tokens, assetsBase));
    prevBg = blockBackground(html) || prevBg;
...
// .ew table, body bg, hidden preheader). `inner` must be a sequence of <tr>…</tr> rows.
function wrapProductionShell(inner, opts = {}) {
  const shell = read(path.join(DS, 'shell', 'shell-production.html'));
  return brandTokens.applyOverrides(shell
...
// Human-readable alt text for a rasterised block slice, derived from the block's own copy
// tokens. NEVER falls back to the component name: internal identifiers in alt text are the
// first thing Gmail's snippet scraper finds in an all-image email, which is how
// "blocks/caption-bar-hero" once led a sent campaign's inbox preview. Markdown markers are
// flattened (alt is an attribute), and a block with no copy tokens gets an empty alt
// (decorative) rather than a slug.
function deriveAlt(tokens = {}) {
  const pick = tokens.HEADLINE || tokens.CAPTION || tokens.PRODUCT_NAME || tokens.PULL_QUOTE
    || tokens.SUPER_LABEL || tokens.SECTION_SUPER || tokens.SUBHEADLINE || '';
  return md.toText(String(pick)).trim();
}
...
function declaresProperty(raw, prop) {
  ...
  const re = new RegExp('(?<![\\w-])' + body, 'i');
  // `max-width` is only a risk where it is the *only* thing setting the width. On an element
  // that also carries a width="…" HTML attribute — which Word honours — dropping max-width
  // changes nothing, so flagging it is noise. sections/full-width-image is exactly this case:
  // measured 600px wide with and without the cap. Checked per tag, since one template can have
  // both an insured element and an exposed one.
  if (prop === 'max-width') {
    return (raw.match(/<[^>]+>/g) || []).some(tag => re.test(tag) && !/\swidth\s*=/.test(tag));
  }
  return re.test(raw);
}

let _htmlOnlyManifest = null;
function htmlOnlyComponents() {
  if (!_htmlOnlyManifest) {
    const manifest = JSON.parse(read(path.join(DS, 'manifest.json')));
    _htmlOnlyManifest = (manifest.assembly && manifest.assembly.html_only_components) || [];
  }
  // A Studio component authored in live-HTML mode joins the same list, so the slice and push
  // paths keep it as real markup instead of flattening it to a PNG. Not cached with the
  // manifest list: publishing a component changes it without a restart.
  return _htmlOnlyManifest.concat(templateSource.htmlOnlyExtras());
}
...
// Every template opens with a COMPONENT/TOKENS/RULES comment, and two callers have to get rid
// of it.
//
// (1) The Outlook scanner below. Those comments discuss the very properties it scans for — a
// component documenting "uses a fixed-width table, not max-width" would report itself broken.
//
// (2) The Klaviyo push path in server.js. Blocks that stay live HTML ship their markup to
// Klaviyo verbatim, author comment and all, and Klaviyo parses template tags INSIDE HTML
// comments. So a RULES line that merely documents a tag ships that tag for real: footer's
// comment mentions {% unsubscribe %} and quietly emits a second, invisible unsubscribe, and a
// bare `{% coupon_code %}` (no coupon name) is invalid — Klaviyo then fails to render the whole
// template and returns 400 on preview/send. Author docs have no business in a sent email
// anyway, so strip them on the way out.
//
// Strip ordinary comments, but keep the conditional ones: `<!--[if gte mso 9]>` wraps real VML, and the downlevel-revealed
// `<!--[if !gte mso 9]><!-->` … `<!--<![endif]-->` form wraps real markup that every other
// client renders.
// The downlevel-revealed markers must go FIRST. `<!--[if !gte mso 9]><!-->` ends in a bare
// `<!--` that a generic comment strip happily treats as the start of a comment, running on to
// the next `-->` and swallowing the very markup the block reveals — which silently hid the
// padded <a> in sections/button from the scanner.
function stripDocComments(html) {
  return html
    .replace(/<!--\[if[^\]]*\]><!-->/g, '')   // downlevel-revealed opener
    .replace(/<!--<!\[endif\]-->/g, '')       // …and its closer
    .replace(/<!--(?!\[if)[\s\S]*?-->/g, ''); // ordinary doc comments; keeps `<!--[if …]>` VML
}
...
function outlookRisks(campaign) {
  ...
  return blocks.map((block, index) => {
    ...
    const markup = stripDocComments(raw);
    const found = OUTLOOK_UNSUPPORTED
      .filter(({ prop }) => declaresProperty(markup, prop))
      .map(({ prop, why }) => ({ property: prop, effect: why }));
    if (PADDED_ANCHOR.test(markup)) found.push({ property: 'padding on <a>', effect: PADDED_ANCHOR_EFFECT });
...
module.exports = { assemble, assembleBlocks, wrapProductionShell, preheader, stripDocComments, openPage, isUnsubscribeBlock, isHtmlOnlyComponent, htmlOnlyComponents, deriveLink, deriveAlt, isGifUrl, withDefaults, renderToPng, renderSlices, closeBrowser, applyOutlookDegradation, outlookRisks, OUTLOOK_UNSUPPORTED, DS };
