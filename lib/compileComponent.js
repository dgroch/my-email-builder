...
// Resolves to the *baseline* palette value, never the current override. A compiled template
// is an immutable artefact; brand overrides are applied at assembly (brandTokens.applyOverrides)
// so one palette edit reaches every component — shipped and authored — without rewriting a
// single published version.
function resolveColour(brand, key, fallbackHex, warnings, where) {
  if (key && Object.prototype.hasOwnProperty.call(brand.baseline.colours, key)) return brand.baseline.colours[key];
